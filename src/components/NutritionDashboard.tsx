...
export function NutritionDashboard({
  ...
  const mealPieData = useMemo(() => {
    return (Object.keys(buckets) as MealBucket[])
      .map((key) => {
        const items = buckets[key];
        const cal = items.reduce((s, l) => s + l.calories, 0);
        return {
          name: t(`mealBuckets.${key}`, key),
          value: cal,
        };
      })
      .filter((d) => d.value > 0);
  ...
  const macroPieData = useMemo(
    () =>
      [
        { name: t("common.carbs", "碳水"), value: totals.carbs * 4 },
        { name: t("common.protein", "蛋白質"), value: totals.protein * 4 },
    ...
    <div className="fixed inset-0 z-[90] bg-zinc-900/50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="w-full max-w-lg bg-zinc-50 rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-4 border-b border-zinc-200 bg-white rounded-t-3xl shrink-0">
          <div>
            <p className="text-xs text-violet-600 font-semibold">{t("nutritionDash.badge", "殿堂級")}</p>
            <h2 className="text-lg font-bold text-zinc-900">{t("nutritionDash.title", "高級營養分析")}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-4 py-4 space-y-4 pb-safe">
          <section className="bg-gradient-to-br from-violet-600 to-indigo-700 text-white rounded-2xl p-4 shadow-lg">
            <p className="text-violet-200 text-xs font-medium mb-3">
              {t("nutritionDash.calories.sectionTitle", "每日熱量動態扣減")}
            </p>
            <div className="grid grid-cols-3 gap-2 text-center text-sm">
              <div className="bg-white/10 rounded-xl py-2">
                <p className="text-white/70 text-[10px]">{t("nutritionDash.calories.goal", "目標 Goal")}</p>
                <p className="font-bold text-lg">{goalCalories}</p>
              </div>
              <div className="bg-white/10 rounded-xl py-2">
                <p className="text-white/70 text-[10px]">{t("nutritionDash.calories.food", "已攝取 Food")}</p>
                <p className="font-bold text-lg">{totals.calories}</p>
              </div>
              <div className="bg-white/10 rounded-xl py-2">
                <p className="text-white/70 text-[10px]">{t("nutritionDash.calories.exercise", "運動 Exercise")}</p>
                <p className="font-bold text-lg">+{exercise}</p>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-white/20 text-center">
              <p className="text-white/80 text-xs">{t("nutritionDash.calories.remaining", "剩餘熱量 Remaining")}</p>
              <p
            ...
            {onExerciseChange && (
              <div className="mt-3 flex gap-2">
                <input
                  ...
                  placeholder={t("nutritionDash.exercise.placeholder", "運動消耗")}
                />
                <button
                  type="button"
                ...
                >
                  {t("nutritionDash.exercise.update", "更新")}
                </button>
              </div>
            )}
          </section>
          ...
          <section className="bg-white rounded-2xl border border-zinc-100 p-4 shadow-sm">
            <h3 className="font-semibold text-zinc-800 mb-3">{t("nutritionDash.charts.title", "圖表分析")}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-zinc-500 text-center mb-2">
                  {t("nutritionDash.charts.mealCalorieShare", "每餐卡路里佔比")}
                </p>
                {mealPieData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      ...
                      <Tooltip formatter={(v) => [`${v} kcal`, t("nutritionDash.tooltip.calories", "熱量")]} />
                      <Legend />
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
                  <p className="text-sm text-zinc-400 text-center py-8">
                    {t("nutritionDash.charts.noMeals", "今日未有飲食記錄")}
                  </p>
                )}
              </div>
              <div>
                <p className="text-xs text-zinc-500 text-center mb-2">
                  {t("nutritionDash.charts.macroRatio", "三大營養素黃金比例")}
                </p>
                {macroPieData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      ...
                      <Tooltip formatter={(v) => [`${v} kcal`, t("nutritionDash.tooltip.energy", "能量")]} />
                      <Legend />
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
                  <p className="text-sm text-zinc-400 text-center py-8">
                    {t("nutritionDash.charts.noMacroData", "未有營養素數據")}
                  </p>
                )}
              </div>
            </div>
                ...
                {recommendations.map((rec, i) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-zinc-500 mt-2">
                      {t("nutritionDash.recommendations.itemLine", "約 {calories} kcal · P{protein} C{carbs} F{fats}", {
