...
import { ChevronRight, LayoutList } from "@/components/icons";
...
const SECTION_LABELS = COACH_STUDENTS_SECTION_LABELS;
  ...
  return (
    <>
      <button
        ...
        className={`relative shrink-0 flex items-center gap-1.5 text-sm font-semibold px-3 py-2.5 rounded-lg min-h-[44px] ${btnClass} text-emerald-800 bg-emerald-50 ring-1 ring-emerald-200/80`}
        aria-haspopup="dialog"
        aria-expanded={open}
        aria-label={`功能選單：${SECTION_LABELS[activeSection]}`}
      >
        <span className="relative shrink-0">
          <LayoutList size={18} strokeWidth={2.25} aria-hidden />
          {unreviewedCount > 0 && isCoach && <UnreadDot />}
        </span>
        <span className="whitespace-nowrap">選單</span>
        <ChevronRight
          size={14}
          strokeWidth={2.5}
          className="shrink-0 rotate-90 text-emerald-600/70"
          aria-hidden
        />
      </button>
